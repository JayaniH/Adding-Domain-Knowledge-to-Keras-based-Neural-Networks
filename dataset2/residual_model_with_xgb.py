...
def train_model(i):
    ...
    df['xgb_latency'] = xgb.predict(i, df[['scenario_passthrough', 'scenario_transformation', 'msg_size', 'concurrent_users']].values.reshape(-1,4))
    df['residuals'] = df['xgb_latency'] - df['avg_response_time']
    ...
    # residual_prediction = residuals = xgb_latency - latency
    # response_time_prediction = xgb_latency  - (xgb_latency - latency)
    response_time_prediction = test['xgb_latency'] - residual_prediction.flatten()
...
def evaluate_model(i):
    ...
    # response_time_prediction = test['xgb_latency'] - scalerY.inverse_transform(residual_prediction).flatten()
    response_time_prediction = test['xgb_latency'] - residual_prediction.flatten()
...
def save_predictions_to_csv(i, test, residual_prediction, latency_prediction):
    results_df = pd.DataFrame({'scenario': test['scenario'], 'msg_size': test['msg_size'], 'concurrent_users': test['concurrent_users'], 'avg_response_time': test['avg_response_time'], 'xgb_latency': test['xgb_latency'],'residuals': test['residuals'] , 'residual_prediction': residual_prediction, 'avg_response_time_prediction': latency_prediction})
    print(results_df)
