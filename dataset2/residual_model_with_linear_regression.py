...
def train_model(i):
    ...
    df['linear_regression_latency'] = linear_regression.predict(i, df[['scenario_passthrough', 'scenario_transformation', 'msg_size', 'concurrent_users']].values.reshape(-1,4))
    df['residuals'] = df['linear_regression_latency'] - df['avg_response_time']
    ...
    # residual_prediction = residuals = linear_regression_latency - latency
    # response_time_prediction = linear_regression_latency  - (linear_regression_latency - latency)
    response_time_prediction = test['linear_regression_latency'] - residual_prediction.flatten()
...
def evaluate_model(i):
    ...
    # response_time_prediction = test['linear_regression_latency'] - scalerY.inverse_transform(residual_prediction).flatten()
    response_time_prediction = test['linear_regression_latency'] - residual_prediction.flatten()

    # remove negative preds
    response_time_prediction = np.maximum(0.0, response_time_prediction)

    _helpers.print_predictions(test['avg_response_time'].values, response_time_prediction)
...
def save_predictions_to_csv(i, test, residual_prediction, latency_prediction):
    results_df = pd.DataFrame({'scenario': test['scenario'], 'msg_size': test['msg_size'], 'concurrent_users': test['concurrent_users'], 'avg_response_time': test['avg_response_time'], 'linear_regression_latency': test['linear_regression_latency'],'residuals': test['residuals'] , 'residual_prediction': residual_prediction, 'avg_response_time_prediction': latency_prediction})
    print(results_df)
