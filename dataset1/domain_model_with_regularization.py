...
def cost(params, X, y_true):
    ...
    param_regularization = 10 * k/l + (s-k)/l
...
def fit_parameters_and_evaluate():
    ...
    for name, group in df:
        ...
        result = minimize(cost, [0,0,1], args=(trainX, trainY), bounds=((0, np.inf), (0, np.inf), (0.00000001, np.inf)))
        # print('result', result)
        print('[INFO] parameter estimates...\t', result.x)
        [s, k, l] = result.x
        ...
        plt.legend()
        # plt.show()
        # plt.savefig('../Plots/30_domain_model_minimization_small_sample/' + name.replace("/", "_") + '.png')
        plt.close()
    ...
    print("Prediction Loss (bucket sampling)\n")
