...
def train_models():
    ...
    for name, group in df:
        ...
        outfile = open("../models/40_ml_approximation_mean_3std/_scalars/scaler_" + name.replace("/", "_") + ".pkl", "wb")
        pkl.dump(scalerx, outfile)
        outfile.close()

        model = models.create_model(trainX.shape[1])
        opt = Adam(learning_rate=1e-2, decay=1e-3/200)
        model.compile(loss=losses.custom_loss_approximation(y_lower, y_upper), optimizer=opt)

        print("[INFO] training model...")
        history = model.fit(x=trainX, y=trainY, validation_data=(testX, testY), epochs=200, batch_size=4)

        # save model
        model.save('../models/40_ml_approximation_mean_3std/' + name.replace("/", "_"))
