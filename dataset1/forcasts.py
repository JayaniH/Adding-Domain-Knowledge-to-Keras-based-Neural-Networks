...
import ml_model_with_custom_loss
...
xgboost_forecasts = regression_xgboost.get_forecast()

ml_forecasts = ml_model.get_forecasts()

domain_test_preds = domain_model.get_forecasts()

residual_model_forecasts = residual_model.get_forecasts()

ml_model_with_custom_loss_forecasts = ml_model_with_custom_loss.get_forecasts()
...
for name, group in df:
    ...
    plt.plot(x, ml_forecasts[name], 'r', label='ML model (NN)')
    # plt.plot(x, xgboost_forecasts[name], 'm', label='XGBoost')
    plt.plot(x, domain_test_preds[name], 'y', label='Domain model')
    plt.plot(x, residual_model_forecasts[name], 'g', label='Residual Model')
    # plt.plot(x, ml_model_with_custom_loss_forecasts[name], 'b', label='ML model with custom loss')
    plt.title(name)
    plt.xlabel('wip')
    plt.ylabel('latency')
    plt.ylim(ymin=0)
