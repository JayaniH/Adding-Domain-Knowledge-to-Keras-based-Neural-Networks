...
def create_model(df, train_i, test_i, i):
    ...
    results_df = pd.DataFrame({'concurrent_users': test['concurrent_users'], 'heap_size': test['heap_size'], 'collector': test['collector'], 'latency': test['latency'], 'prediction': predY})
    # print(results_df)
    results_df.to_csv('../../models/springboot2/domain_model/results/K' + str(i+1) + '.csv', sep=",", index= False)
