...
def train_model(train_i, test_i, i):
    ...
    df['xgb_prediction'] = xgb.predict(i, df[['concurrent_users', 'heap_size', 'collector', 'size',  'use_case']].values.reshape(-1,5))
    df['residual'] = df['xgb_prediction'] - df['latency']
    ...
    # residual_prediction = residual = xgb_prediction - latency
    # latency_prediction = xgb_prediction  - (xgb_prediction - latency)

    print('[INFO] calculating latency predictions...')
    latency_prediction = test['xgb_prediction'] - residual_prediction.flatten()
...
def evaluate_model(train_i, test_i, i):
    ...
    # latency_prediction = test['xgb_prediction'] - scalerY.inverse_transform(residual_prediction).flatten()
...
def save_predictions_to_csv(i, df, residual_prediction, latency_prediction):
    results_df = pd.DataFrame({'concurrent_users': df['concurrent_users'], 'heap_zize': df['heap_size'], 'collector': df['collector'], 'size': df['size'], 'use_case': df['use_case'], 'latency': df['latency'], 'xgb_prediction': df['xgb_prediction'],'residual': df['residual'] , 'residual_prediction': residual_prediction, 'prediction': latency_prediction})
    # print(results_df)
