...
def train_model(train_i, test_i, i):
    ...
    trainX = train[['concurrent_users', 'heap_size', 'collector']].values.reshape(-1,3)
    testX = test[['concurrent_users', 'heap_size', 'collector']].values.reshape(-1,3)
    ...
    outfile = open("../../models/springboot/new_model/xgb_" + str(i+1) + ".pkl", "wb")
...
def evaluate_model(train_i, test_i, i):
    ...
    infile = open("../../models/springboot/new_model/xgb_" + str(i+1) + ".pkl", "rb")
    ...
    results_df = pd.DataFrame({'concurrent_users': test['concurrent_users'], 'heap_size': test['heap_size'], 'collector': test['collector'], 'latency': testY, 'prediction': predY})
    print(results_df)
...
def predict(i, x):
    infile = open("../../models/springboot/6_xgb/xgb_" + str(i+1) + ".pkl", "rb")
