...
def train_model(train_i, test_i, i):
    ...
    df['xgb_prediction'] = xgb.predict(i, df[['concurrent_users', 'heap_size', 'collector']].values.reshape(-1,3))
...
def save_predictions_to_csv(i, df, residual_prediction, latency_prediction):
    results_df = pd.DataFrame({'concurrent_users': df['concurrent_users'], 'heap_zize': df['heap_size'], 'collector': df['collector'], 'latency': df['latency'], 'xgb_prediction': df['xgb_prediction'],'residual': df['residual'] , 'residual_prediction': residual_prediction, 'prediction': latency_prediction})
    print(results_df)
