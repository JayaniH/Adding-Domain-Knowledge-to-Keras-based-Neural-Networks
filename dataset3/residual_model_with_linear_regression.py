...
def train_model(train_i, test_i, i):
    ...
    df['linear_regression_prediction'] = linear_regression.predict(i, df[['concurrent_users', 'cores', 'workload_mix']].values.reshape(-1,3))
...
def save_predictions_to_csv(i, df, residual_prediction, latency_prediction):
    results_df = pd.DataFrame({'concurrent_users': df['concurrent_users'], 'cores': df['cores'], 'workload_mix': df['workload_mix'], 'latency': df['latency'], 'linear_regression_prediction': df['linear_regression_prediction'],'residual': df['residual'] , 'residual_prediction': residual_prediction, 'prediction': latency_prediction})
    print(results_df)
